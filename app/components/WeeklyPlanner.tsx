          ...
          >
            ...
            >
              {!isRest ? (
                <div
                  ...
                  className={cn(
                    ...
                    isLocked ? "bg-primary border-primary text-white scale-110" : "border-white/10 bg-white/5"
                  )}
                >
              ...
              <div className="flex flex-col flex-1">
                ...
                <span className={cn("text-base font-black italic uppercase tracking-tighter transition-colors", isLocked ? "text-primary" : "text-white")}>
                  {day.workout}
                </span>
              </div>
              ...
              {expanded && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
                  <div className={cn(
                    ...
                    isLocked && !saving ? "opacity-60 pointer-events-none" : "opacity-100"
                  )}>
                    
                    {/* HR Section - Rediseñada */}
                    <div className="space-y-4">
                      ...
                      <div className="relative">
                        <input
                          ...
                          placeholder="000"
                          className="w-full bg-white/5 border-2 border-white/10 rounded-2xl px-6 py-2 text-xl font-black italic text-primary placeholder:text-white/10 outline-none focus:border-primary/40 focus:bg-primary/5 transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                        />
                        <span className="absolute right-6 top-1/2 -translate-y-1/2 text-xs font-black text-white/20 uppercase tracking-tighter pointer-events-none">
                          BPM
                        </span>
                      </div>
                      ...
                      <div className="flex gap-2">
                        {[1, 2, 3, 4, 5].map((lvl) => (
                          <button
                            ...
                            className={cn("flex-1 h-11 rounded-xl text-xs font-black transition-all", log.energyLevel === lvl ? "bg-primary text-white shadow-md shadow-primary/20 scale-105" : "bg-white/5 text-muted-foreground")}
                          >
                            {lvl}
                          </button>
                        ))}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        { label: "Feet", key: "footCondition", options: ["Good", "Tired", "Pain"] },
                        { label: "Shins", key: "shinCondition", options: ["Good", "Tight", "Pain"] },
                        { label: "Hips", key: "hipCondition", options: ["Good", "Tight", "Pain"] },
                        { label: "Knees", key: "kneeCondition", options: ["Good", "Weak", "Pain"] },
                      ].map((group) => (
                          ...
                          <div className="flex flex-col gap-1.5">
                            {group.options.map((opt) => (
                              <button
                                ...
                                className={cn("py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all", (log as any)[group.key] === opt ? "bg-white/10 text-white border border-white/10" : "text-muted-foreground hover:text-white")}
                              >
                                {opt}
                              </button>
                            ))}
